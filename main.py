...
def list_courses(x_api_key: Optional[str] = Header(None, alias="X-API-Key")):
    ...
    courses = []
    # 如果你的 pkl 没课程元数据，就退而求其次，前 N 条示例
    for cid, text in list(id_to_text.items())[:20]:
        courses.append(CourseItem(id=int(cid), info=text[:300]))
...
@app.get("/__debug_headers", include_in_schema=False)
def debug_headers(x_api_key: str | None = Header(None, alias="X-API-Key")):
    return {"received": x_api_key or "(none)"}
